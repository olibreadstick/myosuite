...
def record_video(env_name, model_path=curr_dir+'/WheelDist_policy', out_path=curr_dir+"/videos/Wheel_MinDist.mp4"):
    ...
    frames = []
    num_cycles = 3       # How many push-return cycles
    steps_per_push = 500 # Duration of each push
    for _ in range(num_cycles):
        ...
        for _ in range(steps_per_push):
            ...
            # Return hand to grip pose
            env.restore_hand_to_initial_pose()
            # Add some paused frames for visualization
            for _ in range(15):
                frames.append(env.sim.renderer.render_offscreen(width=400, height=400, camera_id=0))
...
if __name__ == "__main__":
    ...
    model = PPO.load('./myosuite/envs/myo/myowheelchair/WithRot_good_grip', envs, verbose = 1, ent_coeff = 0.01, policy_kwargs = policy_kwargs)
